...
class Advising:
    ...
    def heading(self):
        ...
        CurrentDate_Label = Label(InfoHeading, text = "" + str(d.month) + " /"+ str(d.day) + " /" + str(d.year))
    ...
    def submit(self):
        ...
        if not self.var.get():
            messagebox.showerror("Error","Please choose an advisor")
            return
            
        if not self.id.get():
            messagebox.showerror("Error","Please write your ID #")
            return
            
        if not self.term1.get() or not self.term2.get() or not self.term3.get():
            messagebox.showerror("Error","Please fill out the three semester fields")
            return
